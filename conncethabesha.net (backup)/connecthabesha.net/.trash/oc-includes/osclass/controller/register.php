  ...
  public function doModel() {
    switch($this->action) {
      ...
      case('register_post'):  // register user action (form submitted)
        ...
        if($success == 1) {
            $method_s = Params::getParam('s_method');
            if($method_s==1){
                // osc_add_flash_ok_message(_m('Your account has been created successfully'));
                osc_add_flash_ok_message(_m('The user has been created. Please verify your phone number to activate your account.')); 
                ...
                $do = new CWebLogin();
                $do->doModel($method_s);
                //osc_add_flash_ok_message(_m('The user has been created. Please login and verify your phone number to activate your account.')); 
            }else{
